<template>
  <div v-if="activeVault">
    ...
    <v-toolbar height="90rem" app>
      ...
      <v-btn style="margin-top: 1rem;" @click="$router.push({ name: 'dashboard' })" class="back-button">
    ...
    <div class="page-wrapper">
      ...
      <!-- HEADER -->
      <section>
        <h1 class="upper heading">You're visiting the <br/> {{activeVault.name}} vault</h1>
      </section>
      ...
      <!-- VIEW VAULT CARD -->
      <section>
        <v-card class="dashboard-vault">
          <v-container row wrap>
            <v-flex xs12>
              <v-card-title class="upper card-heading">
                <p style="margin-top: 1.8rem" class="vault-description">{{activeVault.description}}</p>

                <v-spacer></v-spacer>

                <v-btn @click="addKeepDialogVal = true" color="#2ec4b6" fab>
                  <v-icon color="#ffffff">add</v-icon>
                </v-btn>
                <v-btn @click="editVaultDialogVal = true" color="#2ec4b6" fab>
                  <v-icon color="#ffffff">edit</v-icon>
                </v-btn>
                <v-btn @click="deleteVaultDialogVal = true" color="#2ec4b6" fab>
                  <v-icon color="#ffffff">delete_outline</v-icon>
                </v-btn>
              </v-card-title>
              ...
              <!-- KEEPS INSIDE OF VAULT -->
              <div class="vaultKeeps">
                <v-layout v-if="vaultKeeps.length > 0">
                  <keeps :keeps="vaultKeeps"/>
                </v-layout>

                <div v-else class="no-keeps">
                  <p class="upper">You currently don't have any keeps in this vault</p>
                </div>
      ...
      <v-dialog scrollable v-model="addKeepDialogVal" max-width="400">
        <v-card class="form">
          <v-card-title style="margin-top: .5rem;" class="upper">Select a keep to add to vault:</v-card-title>

          <v-divider style="margin: 1rem;" color="#2ec4b6"></v-divider>

          <v-card-text style="height: 400px;">
            <v-radio-group column>
              <v-radio v-for="keep in userKeeps" :key="keep.id" :label="keep.name" :value="keep.name"></v-radio>
            </v-radio-group>
          ...
          <v-card-actions>
            ...
            <v-btn @click="addKeep(keep)">Add</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>

      <!-- EDIT THE VAULT DIALOG -->
      <v-dialog v-model="editVaultDialogVal" width="800">
        <v-card class="form">
          <v-card-title class="upper text-bold">Edit this vault</v-card-title>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn @click="editVaultDialogVal = false" class="upper">Cancel</v-btn>
            <v-btn type="submit" @click="editVault(this.editVaultDialog); editVaultDialogVal = false;" class="upper">edit vault</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>

      <!-- DELETE VAULT DIALOG -->
      <v-dialog v-model="deleteVaultDialogVal" width="800">
        <v-card class="form upper">
          <v-card-title> Warning! </v-card-title>

          <v-divider color="#2ec4b6"></v-divider>

          <v-card-text>Are you sure you want to delete this vault?</v-card-text>

          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn @click="deleteVaultDialogVal = false" class="upper">Cancel</v-btn>
            <v-btn @click="deleteVault(activeVault)" class="upper">Delete</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("getKeepsByVault", this.$route.params.id);
  },
  methods: {
    logout() {
